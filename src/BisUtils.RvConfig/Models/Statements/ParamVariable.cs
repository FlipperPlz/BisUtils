...
public abstract class ParamVariableBase : ParamStatement, IParamVariable
{
    ...
    protected ParamVariableBase(IRvConfigFile file, IParamStatementHolder parent, ILogger? logger) : base(file, parent, logger)
    {
    }

    protected ParamVariableBase(BisBinaryReader reader, ParamOptions options, IRvConfigFile file, IParamStatementHolder parent, ILogger? logger) : base(reader, options, file, parent, logger)
    {
    }
...
public class ParamVariable : ParamVariableBase, IParamVariable
{
    ...
    public ParamVariable(string variableName, IParamLiteral variableValue, ParamOperatorType operatorType, IRvConfigFile file, IParamStatementHolder parent, ILogger? logger) : base(file, parent, logger)
    {
    ...
    public ParamVariable(BisBinaryReader reader, ParamOptions options, IRvConfigFile file, IParamStatementHolder parent, ILogger? logger) : base(reader, options, file, parent, logger)
    {
    ...
    public sealed override Result Debinarize(BisBinaryReader reader, ParamOptions options)
    {
        ...
        VariableName = name;
        result.WithReasons(ParamLiteralFactory.ReadLiteral(reader, options, out var value, RvConfigFile, this, Logger).Reasons);
        VariableValue = value!;
        return result;
    }
