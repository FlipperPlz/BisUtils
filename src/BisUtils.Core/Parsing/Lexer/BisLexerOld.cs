...
public interface IBisLexerOld<TTokenEnum> where TTokenEnum : Enum
{
    /// <summary>
    ...
    /// <param name="lexerOld">The lexer instance handling the input.</param>
    public delegate void TokenMatched(TokenMatch match, IBisLexerOld<TTokenEnum> lexerOld);
...
public abstract class BisLexerOld<TTokenEnum> : BisMutableStringStepper, IBisLexerOld<TTokenEnum> where TTokenEnum : Enum
{
    public event IBisLexerOld<TTokenEnum>.TokenMatched? OnTokenMatched;

    /// <summary>
    /// An abstract member representing the defined token types in lexer.
    /// </summary>
    public abstract IEnumerable<IBisLexerOld<TTokenEnum>.TokenDefinition> TokenTypes { get; }

    /// <summary>
    /// An abstract member representing Error token type in lexer.
    /// </summary>
    public abstract IBisLexerOld<TTokenEnum>.TokenDefinition? ErrorToken { get; }

    /// <summary>
    /// An abstract member representing EOF token type in lexer.
    /// </summary>
    public abstract IBisLexerOld<TTokenEnum>.TokenDefinition EOFToken { get; }

    /// <summary>
    /// A list of previously matched tokens.
    /// </summary>
    public IEnumerable<IBisLexerOld<TTokenEnum>.TokenMatch> PreviousMatches => previousMatches;
    private readonly List<IBisLexerOld<TTokenEnum>.TokenMatch> previousMatches = new();

    protected BisLexerOld(string content) : base(content)
    {
    }
    ...
    /// </summary>
    /// <returns>The previously matched token.</returns>
    public IBisLexerOld<TTokenEnum>.TokenMatch? PreviousMatch() => PreviousMatches.LastOrDefault();
    ...
    public IBisLexerOld<TTokenEnum>.TokenMatch NextToken()
    {
    ...
    public IBisLexerOld<TTokenEnum>.TokenMatch TokenizeWhile(TTokenEnum type, bool reverse = false)
    {
        IBisLexerOld<TTokenEnum>.TokenMatch token;
        if (reverse)
        {
    ...
    /// <param name="match">The matched token.</param>
    /// <param name="lexerOld">The lexer instance handling the input.</param>
    protected virtual void OnTokenMatchedHandler(IBisLexerOld<TTokenEnum>.TokenMatch match, IBisLexerOld<TTokenEnum> lexerOld)
    {
        previousMatches.Add(match);
        OnTokenMatched?.Invoke(match, lexerOld);
    }

    /// <summary>
    /// Creates a token definition..
    /// </summary>
    protected static IBisLexerOld<TTokenEnum>.TokenDefinition CreateTokenDefinition(string debugName, TTokenEnum tokenType, short tokenWeight) =>
    ...
    /// </summary>
    protected IBisLexerOld<TTokenEnum>.TokenMatch CreateTokenMatch(Range tokenRange, IBisLexerOld<TTokenEnum>.TokenDefinition tokenDef) =>
        new() {
            Success = true,
    ...
    /// </summary>
    protected static IBisLexerOld<TTokenEnum>.TokenMatch CreateTokenMatch(Range tokenRange, string str, IBisLexerOld<TTokenEnum>.TokenDefinition tokenDef) =>
        new() {
            Success = true,
            TokenLength = tokenRange.End.Value - tokenRange.Start.Value + 1,
            TokenPosition = tokenRange.Start.Value,
    ...
    /// </summary>
    /// <returns>The next token.</returns>
    protected abstract IBisLexerOld<TTokenEnum>.TokenMatch GetNextToken();
