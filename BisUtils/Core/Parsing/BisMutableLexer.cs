...
public class BisMutableLexer : BisLexer
{
    ...
    public void ReplaceRange(Range range, string replacement)
    {
        ...
        if (start < 0 || start > Content.Length) throw new ArgumentOutOfRangeException(nameof(range), "Starting index is out of bounds.");
        if (end < start || end > Content.Length) throw new ArgumentOutOfRangeException(nameof(range), "Ending index is out of bounds.");
